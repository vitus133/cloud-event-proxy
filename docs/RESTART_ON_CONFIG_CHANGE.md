...
**Key point**: These are two different things. The daemon monitors the ConfigMap (mounted file), and when it changes, the daemon reads it, generates new config files in `/var/run/`, and sends `RESTART` to coordinate the sidecar reload.
...
**Single restart trigger.** The daemon is the single source of truth for when the sidecar should restart. The `RESTART` socket command is the only trigger. The sidecar does not watch the ConfigMap or config files for changes.
...
This guarantees consistent state: the ConfigMap and config files always match because the daemon writes them before sending `RESTART`.

**Startup synchronization without handshake.** The daemon guarantees that config files are written before sending `RESTART`. On initial startup, `loadInitialPtp4lConfigs()` may find no configs (graceful). Once the daemon writes configs and sends `RESTART`, the sidecar restarts and reads them. No handshake protocol needed.
...
When the daemon decides a restart is needed, it sends a single `RESTART` line on the same socket. The sidecar handles it inline in `processMessages()`.
...
The existing socket at `/cloud-native/events.sock` sends one line per message. We add a single reserved command:
...
**No READY message needed.** The daemon guarantees that config files are written before sending `RESTART`. On initial startup, if the sidecar starts before the daemon has written configs, `loadInitialPtp4lConfigs()` simply finds no configs (empty list). When the daemon finishes and sends `RESTART`, the sidecar restarts and reads them.

### Backward Compatibility

- **Old daemon, new sidecar**: Daemon never sends `RESTART`. Sidecar loads configs once on startup and processes log lines normally. No restart occurs, but no crash either.
- **New daemon, old sidecar**: Daemon sends `RESTART`. Old sidecar passes it to `ExtractMetrics()` which ignores it (not a recognized log line). No harm.
...
**Rationale**: ConfigMap changes are detected by the daemon, not the sidecar. The daemon sees the change (it has the ConfigMap mounted), writes new config files, and sends `RESTART`. The sidecar restart reads both the updated ConfigMap and config files atomically.
...
- `restartProcess()` function (triggered by socket `RESTART` instead of ConfigMap change)
- `WatchConfigMapUpdate` (runs once for initial ConfigMap load)
- `listenToSocket` / `processMessages` (enhanced with `RESTART` handling)
...
4. Daemon finishes writing initial configs and sends `RESTART`
5. Sidecar restarts via `syscall.Exec`
6. `Start()` calls `loadInitialPtp4lConfigs()` again, now finds configs

No special handling needed. The system self-corrects on first `RESTART`.
...
4. Daemon writes configs and sends `RESTART`
...
**Scenario**: Daemon deletes config files but doesn't send `RESTART` (daemon bug or crash).

**Behavior**: Sidecar continues processing log lines with stale interface mappings. Metrics may report stale data.

This is acceptable. The daemon should always send `RESTART` after config changes. If it doesn't, this is a daemon bug to fix, not a sidecar problem. The sidecar trusts the daemon.

### Multiple rapid restarts

**Scenario**: Daemon sends multiple `RESTART` commands in quick succession.

**Behavior**:
- First `RESTART` triggers `restartProcess()`, which calls `syscall.Exec`
- The old process image is replaced; socket buffer is discarded
- The new process starts fresh, reads current configs
- If daemon sends another `RESTART` later, the new process restarts again
...
2. **`processMessages()` RESTART handling**:
   - Mock socket sending `RESTART\n`
   - Verify `restartProcess()` is called
   - Verify log lines before/after `RESTART` still reach `ExtractMetrics()`
...
|-------------|----------|
| Old daemon + new sidecar | Daemon never sends `RESTART`. Sidecar loads configs once and processes log lines. No restart, no crash. |
| New daemon + old sidecar | Daemon sends `RESTART`. Old sidecar passes it to `ExtractMetrics()` which ignores it. No harm. |
...
The linuxptp-daemon (separate repository) must be updated to send `RESTART\n` on the Unix socket `/cloud-native/events.sock` after completing a config change.

### Where to Send RESTART

The `RESTART` command should be sent at the end of the `ApplyNodePTPProfiles()` function in the daemon. This function is the single place where the daemon:
...
Since `RESTART` is sent at the end of `ApplyNodePTPProfiles()`, it covers all config change scenarios:

1. **ConfigMap change**: Daemon detects updated ConfigMap, calls `ApplyNodePTPProfiles()`, sends `RESTART` at the end
2. **Initial startup**: Daemon applies initial profiles via `ApplyNodePTPProfiles()`, sends `RESTART` at the end
3. **Profile deletion**: Daemon removes profiles via `ApplyNodePTPProfiles()`, sends `RESTART` at the end
...
| Before | After |
|--------|-------|
| Two independent restart triggers (fsnotify + ConfigMap polling) | One restart trigger (daemon `RESTART` command) |
